...
# .zebrafish.zsh
# version: 0.5.0
# homepage: https://github.com/zshzoo/zebrafish
#
# A solid base zsh configuration achieved with only one small, simple include.
#
# single file install:
#   curl -s -o ${ZDOTDIR:-$HOME}/.zebrafish.zsh https://raw.githubusercontent.com/zshzoo/zebrafish/master/.zebrafish.zsh
...
_features=
_zebrafish_dir
if zstyle -T ':zebrafish:core' 'dir'; then
  if [[ -n "$ZDOTDIR" ]]; then
    zstyle ':zebrafish:core' 'dir' "$ZDOTDIR"
  else
    zstyle ':zebrafish:core' 'dir' "${XDG_CONFIG_DIR:-$HOME/.config}/zebrafish"
  fi
fi
zstyle -g ZEBRAFISHDIR ':zebrafish:core' 'dir'
...
_zf_init() {
  ...
  # set defaults
  if zstyle -T ':zebrafish:features:plugins' 'dirname'; then
    zstyle ':zebrafish:features:plugins' 'dirname' "plugins"
  fi
  ...
  # get variables for all the necessary feature dirs
  zstyle -g zfuncs_dirname ':zebrafish:features:zfunctions' 'dirname'
  _zfuncs_path="${_zebrafish_path}/${zfuncs_dirname}"

  zstyle -g plugins_dirname ':zebrafish:features:plugins' 'dirname'
  _plugins_path="${_zebrafish_path}/${plugins_dirname}"
  export PLUGZ_DIR="${_plugins_path}"
...
_zf_plugins() {
  if [[ ! -d "$PLUGZ_DIR" ]]; then
    git clone -b develop https://github.com/zshzoo/plugz.git "$PLUGZ_DIR"
  fi
  PLUGZ_RESET_ON_CHANGE=(${ZDOTDIR:-$HOME}/.zshrc ${ZDOTDIR:-$HOME}/.zplugins)
  PLUGZ_AUTOLOAD_COMPINIT=false

  if [[ ! -f ${ZDOTDIR:-$HOME}/.zplugins ]]; then
    cat <<EOF > ${ZDOTDIR:-$HOME}/.zplugins
...
EOF
  ...
  # load plugins
  source "$PLUGZ_DIR/plugz.zsh"
  if ! plugz saved; then
    # use zplugins file for other plugins if it exists
    if [[ -f ${ZDOTDIR:-$HOME}/.zplugins ]]; then
      plugz loadall < ${ZDOTDIR:-$HOME}/.zplugins
    fi

    # save plugz plugins into init.zsh
    plugz save
  fi
}
...
_zf_zfuncs() {
  ...
  fpath=("$_zfuncs_path" $fpath)
  for zfunc in "$_zfuncs_path"/*(N); do
    autoload -U "$zfunc"
  done
}
...
# ------------------------------------------------------------------------------
_zf_zshrcd() {
  local zfile zshrcd_dirname zshrcd_path
  zstyle -g zshrcd_dirname ':zebrafish:features:zshrc.d' 'dirname'
  zshrcd_path="${_zebrafish_path}/${zshrcd_dirname}"
